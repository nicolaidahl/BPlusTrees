...
A vital aspect of B+ trees is that all of the key-point and key-value lists in nodes and leaves are sorted by the key. $kvl\_sorted$ is a proposition that only holds if the list it is applied to is sorted. The proposition is reproduced in Fig. \ref{fig:kvl_sorted}.
