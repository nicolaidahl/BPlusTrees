...
Lemma insert_leaf_split_preserves_max_fan_out : forall {X: Type} b counter k v t n l l', 
  (t, Some (n, bptLeaf b X l')) = insert' counter k v (bptLeaf b X l) ->
  length l' <= b * 2.
Proof. Admitted.

Lemma insert_leaf_preserves_max_fan_out : forall {X: Type} b counter k v o l l', 
  (bptLeaf b X l', o) = insert' counter k v (bptLeaf b X l) ->
  length l' <= b * 2.
Proof.
  ...
  Case "blelen true".
    ...
    apply insert_into_list_max_grows_one. destruct H0. admit. admit.
...
Lemma insert_leaf_preserves_sort : forall {X: Type} b counter k v o l l', 
  kvl_sorted l -> (bptLeaf b X l', o) = insert' counter k v (bptLeaf b X l) ->
  kvl_sorted l'.
Proof. Admitted.

Lemma insert_leaf_split_preserves_min_fan_out : forall {X: Type} b counter k v t n l l', 
  (t, Some (n, bptLeaf b X l')) = insert' counter k v (bptLeaf b X l) ->
  length l' >= b.
Proof. Admitted.
...
Lemma insert_leaf_split_preserves_sort_right : forall {X: Type} b counter k v t n l l', 
  kvl_sorted l -> (t, Some (n, bptLeaf b X l')) = insert' counter k v (bptLeaf b X l) ->
  kvl_sorted l'.
Proof. Admitted.


Lemma insert_leaf_split_never_node : forall {X: Type} b counter k v t o n l l', 
  ((t, Some (n, bptNode b X l')) = insert' counter k v (bptLeaf b X l) \/
  (bptNode b X l', o) = insert' counter k v (bptLeaf b X l)) ->
  False.
Proof. Admitted.

Lemma insert_leaf_split_ensures_n_positive : forall {X: Type} b counter k v n t l l',
  kvl_sorted l -> (t, Some (n, l')) = insert' counter k v (bptLeaf b X l) ->
  n > 0.
Proof.
Admitted.

Lemma split_leaf_impl_length_max_fan_out: forall {X: Type} b counter t1 t2 n k v l,
   (t1, Some (n, t2)) = insert' counter k v (bptLeaf b X l) ->
   length l = S (b * 2).
Proof. Admitted.
